...
- **🐋 The leviathan** — A vast apex shadow with a cold bioluminescent underbelly and a pale anglerfish lure prowls the deep, hunting *the predators themselves*. When it seizes a hunter it booms a sub-bass note and flings it, shrunken, into the far dark — so the dark has its own predator, and the whole thing settles into a three-tier food chain that balances itself.
- **🌱 A living lifecycle** — Every creature is born small and dim, brightens as it grows into adulthood, fades a little in old age, and eventually passes on in a soft expanding glimmer — replaced by a new young soul that drifts in from the edge. The swarm is never a fixed set of identical dots; it's a population of mixed ages turning over in real time.
- **🎬 Self-demoing attract mode** — Leave it untouched and a ghost hand stirs the water and breathes blooms into the dark on its own, so the piece is alive before you ever touch it. The moment you move, tap or drag, you take the helm and the prompt dissolves.
...
**The food chain.** The leviathan applies the exact same trick one tier up: it *stalks* toward the nearest predator and strikes from farther (205px) than the predator dreads it (150px), lunging faster than a fleeing hunter can run. The emergent result is a self-balancing rhythm — when the leviathan drives the predator off, boid predation pauses; as the predator returns, the swarm thins again. Watch the `taken by the dark` and `hunters scattered` counters move in counterpoint.

**Lifecycle.** Each creature carries an `age` and a randomised `maxAge`. A `grow` factor (age over the first ~16% of life) ramps its size and luminosity up from a dim newborn to a full adult; past ~82% of life an `eld` factor dims it again. When age crosses `maxAge` it leaves a soft glimmer (a "mote" that expands and fades) and is reborn through the same edge-respawn the predators use — with a fresh size, brightness and lifespan, but its original hue, so it returns to its tribe. Initial ages are staggered across the whole lifespan so the population starts mixed and turns over gradually rather than all at once.

**Attract mode.** Until the first genuine pointer event, a scripted "ghost hand" follows a slow Lissajous path and periodically drops a bloom, driving the live simulation so an idle screen demonstrates itself. The first real interaction flips a flag that hands control back and fades the prompt — the simulation never restarts, it just changes who's steering.
